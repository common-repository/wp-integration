 ...
 *  @copyright 2009-2018 Inveo s.r.o.
 *  @license   LGPLv2.1
 */
...
{
	echo 'Hi there! I\'m just a WordPress plugin, not much I can do when called directly.';
	return;
}
...
define('THEMEPROVIDER_CONN_VERSION', '1.6.00');
...
{
	if(version_compare(THEMEPROVIDER_VERSION, '1.5.00', '>='))
	{
...
if (is_admin()) // admin actions
{
	...
	if(version_compare(get_bloginfo('version'), '2.7', '>='))
	{
		...
		if(WebAppsDetector::appFound() && WebAppsDetector::providerFound() && basename($_SERVER['SCRIPT_NAME']) == 'plugins.php' && defined('THEMEPROVIDER_VERSION') && version_compare(THEMEPROVIDER_VERSION, WebAppsDetector::getLatest(), '<'))
	...
	defined('THEMEPROVIDER_INIT') && defined('WP_USE_THEMES') && !defined('XMLRPC_REQUEST') && !defined('REST_REQUEST') && !defined('WP_INSTALLING')
	&& (
		(function_exists('wp_doing_ajax') && !wp_doing_ajax())
			||
		(!function_exists('wp_doing_ajax') && !defined('DOING_AJAX'))
	)
...
copyright &copy; 2012-<?php echo date('Y'); ?> Inveo
